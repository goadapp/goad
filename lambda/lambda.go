...
var (
	...
	address        = app.Flag("url", "URL to load test").Short('u').Required().String()
	...
	requestHeaders = app.Flag("headers", "List of headers").Short('H').Strings()

	awsRegion   = app.Flag("awsregion", "AWS region to run in").Short('r').String()
	queueRegion = app.Flag("queueRegion", "SQS queue region").Short('q').String()
	sqsURL      = app.Flag("sqsurl", "SQS URL").Short('s').String()

	clientTimeout      = app.Flag("client-timeout", "Request timeout duration").Short('t').Default("15s").Duration()
	...
	execTimeout                   = app.Flag("execution-time", "Maximum execution time in seconds").Short('N').Default("0").Int()
)
